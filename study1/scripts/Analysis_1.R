...
levels(as.factor(xdata$condition.trial))
...
ftable(searched.after ~ 
        condition.trial + reward.received, xdata)
xdata$condition.trial = 
 factor(xdata$condition.trial, 
...
xx.fe.re=fe.re.tab(fe.model="searched.after ~ 
                   age*reward.received + trial.per.child",
...
t.data$condition.trial.no.choice.code = 
 t.data$condition.trial.no.choice - mean(t.data$condition.trial.no.choice)
t.data$condition.trial.choice.code = 
 t.data$condition.trial.choice - mean(t.data$condition.trial.choice)
...
full <- glmer(searched.after ~ reward.received*condition.trial*z.age +
               (1 + (reward.received.low.code + reward.received.none.code) | id), 
              data = t.data, control = contr, 
              family = binomial(link = "logit"))

red <- glmer(searched.after ~ (reward.received+condition.trial+z.age)^2 +
              (1 + (reward.received.low.code + reward.received.none.code) | id), 
             data = t.data, control = contr, 
             family = binomial(link = "logit"))

red2 <- glmer(searched.after ~ reward.received + condition.trial*z.age +
               (1 + (reward.received.low.code + reward.received.none.code) | id), 
              data = t.data, control = contr, 
              family = binomial(link = "logit"))

main <- glmer(searched.after ~ reward.received + condition.trial + z.age +
               (1 + (reward.received.low.code + reward.received.none.code) | id), 
              data = t.data, control = contr, 
              family = binomial(link = "logit"))

null <- glmer(searched.after ~ 1 +
               (1 + (reward.received.low.code + reward.received.none.code) | id), 
              data = t.data, control = contr, 
              family = binomial(link = "logit"))
...
ftable(searched.after ~ condition.trial + reward.received, t.data)
ftable(searched.after ~ condition.trial + age.group, t.data)
ftable(searched.after ~ reward.received, t.data)
...
tests.full=drop1p(model.res=full, para=F, data=NULL, 
                  contr=contr, n.cores=c("all-1", "all"), to.del=NULL)
round(tests.full$drop1.res, 3) 
tests.red=drop1p(model.res=red, para=F, data=NULL, 
                 contr=contr, n.cores=c("all-1", "all"), to.del=NULL)
round(tests.red$drop1.res, 3) 
tests.red2=drop1p(model.res=red2, para=F, data=NULL, 
                  contr=contr, n.cores=c("all-1", "all"), to.del=NULL)
round(tests.red2$drop1.res, 3)
tests.main=drop1p(model.res=main, para=F, data=NULL, 
                  contr=contr, n.cores=c("all-1", "all"), to.del=NULL)
round(tests.main$drop1.res, 3)

## First peek at effects
library("effects")
plot(effect("condition.trial:z.age", red2))
plot(effect( c("reward.received") , main))

## Pairwise comparisons
emm1 = emmeans(red2,  ~ condition.trial)
summary(emm1, type = "response")
emmeans(red2, pairwise ~ condition.trial)
...
boot.full=boot.glmm.pred(model.res=full, excl.warnings=T,
                         nboots=1000, para=F, level=0.95, use=c("condition.trial", "reward.received", "z.age"))
...
model.condition.age <- glmer(searched.after ~ condition.trial*z.age + (reward.received.low.code + reward.received.none.code) + 
                              (1 | id), data = t.data, control = contr, 
                             family = binomial(link = "logit"))
boot.condition.age=boot.glmm.pred(model.res=model.condition.age, excl.warnings=T,
                                  nboots=1000, para=F, level=0.95, use=c("condition.trial", "z.age"))
...
model.reward <- glmer(searched.after ~ reward.received + z.age*(condition.trial.no.choice.code + condition.trial.choice.code) +
                       (1 | id), data = t.data, control = contr, 
                      family = binomial(link = "logit"))
boot.reward=boot.glmm.pred(model.res=model.reward, excl.warnings=T,
                           nboots=1000, para=F, level=0.95, use=c("reward.received"))
...
exp1_plot_reward <- 
 ggplot(data = xdata.agg, aes(x = factor(reward.received, 
                          levels=c("high", "low", "none")), y = mean.resp2)) +
 
 # geom_line(aes(x = reward.received2, y = mean.resp2, group = id), 
 #           color = "gray", lty = 1, alpha = .3) +
 
 geom_point(data = xdata.agg %>% filter(reward.received == "high"), 
            aes(x = reward.received2), color = "chartreuse4", size = 1.5, 
            alpha = .6) +
 geom_point(data = xdata.agg %>% filter(reward.received == "low"), 
            aes(x = reward.received2), color = "gold3", size = 1.5, 
            alpha = .6) +
 geom_point(data = xdata.agg %>% filter(reward.received == "none"), 
            aes(x = reward.received2), color = "red3", size = 1.5, 
            alpha = .4) +
 
 geom_violin(data = xdata.agg %>% filter(reward.received == "high"),
                   aes(x = reward.received, y = mean.resp),
                   position = position_nudge(x = 0.00),
              fill = "chartreuse4", alpha = .2) +
 geom_violin(data = xdata.agg %>% filter(reward.received == "low"),
                   aes(x = reward.received, y = mean.resp),
                   position = position_nudge(x = 0.00),
             fill = "gold3", alpha = .2) +
 geom_violin(data = xdata.agg %>% filter(reward.received == "none"),
                   aes(x = reward.received, y = mean.resp),
                   position = position_nudge(x = 0.00),
                   fill = "red3", alpha = .2) +
 ...
 geom_errorbar(data = boot.reward$ci.predicted %>% 
               ...
               aes(x = as.numeric(reward.received) + 0.25, y = fitted, 
                   ymin = lower.cl, ymax = upper.cl), color = "chartreuse4", 
                   width = 0.1, size = 1) +
 geom_errorbar(data = boot.reward$ci.predicted %>% 
                filter(reward.received == "low"), 
               aes(x = as.numeric(reward.received) + 0.25, y = fitted, 
                   ymin = lower.cl, ymax = upper.cl), color = "gold3", 
                   width = 0.1, size = 1) +
 geom_errorbar(data = boot.reward$ci.predicted %>% 
                filter(reward.received == "none"), 
               aes(x = as.numeric(reward.received) + 0.25, y = fitted, 
                   ymin = lower.cl, ymax = upper.cl), color = "red3", 
                   width = 0.1, size = 1) +
 ...
 scale_y_continuous(name = "Proportion of trials with information-search", labels = scales::percent) +
...
xdata.agg.condition <- xdata %>%
 mutate(searched.after.numeric=as.numeric(searched.after)-1)%>%
 group_by(id, z.age, condition.trial) %>%
 summarise(mean.resp = mean(searched.after.numeric, na.rm = T)) %>%
 ungroup()
...
#exp1_plot_condition <- 
 ggplot() +
      geom_point(data = xdata.agg.condition, 
             aes(x = z.age, y = mean.resp2, color = condition.trial),
            size = 2.5, alpha = .4) +
      scale_color_manual(values=c("dodgerblue", "darkorange", "darkorange")) +
      geom_ribbon(data = boot.condition.age$ci.predicted, 
                  aes(ymin = lower.cl, ymax = upper.cl, x = z.age, 
                       group = condition.trial,
                       fill = condition.trial), 
      ...
      facet_wrap(~condition.trial, switch = "x") +
      scale_x_continuous(name="Condition * Age", 
                             breaks = c((2-mean(xdata$age))/sd(xdata$age), 
