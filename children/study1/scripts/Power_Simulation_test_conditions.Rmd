...
conditions <- rep(c(".no.choice", ".choice"), each = n.subject / n.condition)
start.data$condition <- factor(conditions[as.numeric(start.data$subj.id)],
                               levels = c(".no.choice", ".choice"))
...
for (i in 1:nrow(all.res)) {
  ...
  ## store results:
  all.res[i, c(
    "icpt", "condition.choice", "z.age", "value.low", "value.none", 
    "condition.choice.z.age" , "condition.choice.value.low",
    "condition.choice.value.none", "z.age.value.low", "z.age.value.none"
  )] <- fixef(full$value)
...
save.image("./study1/R_objects/counterfactual_power_sim_test_only.RData")
load("./study1/R_objects/counterfactual_power_sim_test_only.RData")
```
...
all.res1 <- all.res
tapply(
  X = all.res1[, "warns.full"] > 0, INDEX = all.res1[, c("r.slope.value.low", "r.slope.value.none")],
  FUN = sum
)
...
lrt.data1 <- all.res2 %>%
  group_by(
    r.slope.value.low, r.slope.value.none
  ) %>%
  summarise(
