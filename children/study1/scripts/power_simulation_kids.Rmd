---
title: "Study_1_Power_Simulation_Kids"
author: "Hanna Schleihauf & Lou Haux"
date: "3/10/2022"
output: pdf_document
---
...
n.subject <- 108 # number of subjects
min.age <- 2 # range of age (might need to be adjusted)
max.age <- 4
n.per.subject <- 4 # observations per subject
n.condition <- 3 # number of conditions
n.per.condition <- 4 # observations per subject and condition (between-subject design)
...
control.2 <- 0.10 # <- reference level
choice.2 <- 0.40 
no.choice.2 <- 0.50
...
start.data$condition <-
  as.factor(rep(x = c(".control", ".choice", ".no.choice"), each = n.subject / 3))[as.numeric(start.data$subj.id)]

table(start.data$condition, start.data$trial.per.condition)

  condition = c(".control", ".choice", "no-choice"),
